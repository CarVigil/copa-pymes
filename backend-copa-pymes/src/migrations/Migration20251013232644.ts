...
export class Migration20251013232644 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`create table \`division\` (\`id\` int unsigned not null auto_increment primary key, \`nombre\` varchar(255) null, \`cupo\` varchar(255) null) default character set utf8mb4 engine = InnoDB;`);

    this.addSql(`create table \`equipo\` (\`id\` int unsigned not null auto_increment primary key, \`nombre\` varchar(255) not null, \`sigla\` varchar(255) not null, \`estado\` tinyint(1) not null default true, \`escudo\` longtext null) default character set utf8mb4 engine = InnoDB;`);

    this.addSql(`create table \`sede\` (\`id\` int unsigned not null auto_increment primary key, \`nombre\` varchar(255) null, \`ubicacion\` varchar(255) null, \`capacidad\` varchar(255) null) default character set utf8mb4 engine = InnoDB;`);

    this.addSql(`create table \`torneo\` (\`id\` int unsigned not null auto_increment primary key, \`nombre\` varchar(255) not null, \`tipo\` varchar(255) not null, \`modalidad\` varchar(255) not null, \`cantidad_divisiones\` int null, \`cantidad_equipos\` int null, \`fecha_inicio\` date null, \`fecha_fin\` date null, \`estado\` varchar(255) not null default 'pendiente') default character set utf8mb4 engine = InnoDB;`);

    this.addSql(`create table \`premio\` (\`id\` int unsigned not null auto_increment primary key, \`fecha_entrega\` datetime not null, \`torneo_id\` int unsigned not null, \`nombre\` varchar(255) not null, \`descripcion\` varchar(255) null, \`posicion\` varchar(255) not null) default character set utf8mb4 engine = InnoDB;`);
    this.addSql(`alter table \`premio\` add index \`premio_torneo_id_index\`(\`torneo_id\`);`);

    this.addSql(`create table \`inscripcion\` (\`id\` int unsigned not null auto_increment primary key, \`torneo_id\` int unsigned not null, \`equipo_id\` int unsigned not null, \`estado\` varchar(255) not null default 'pendiente', \`fecha_inscripcion\` date null) default character set utf8mb4 engine = InnoDB;`);
    this.addSql(`alter table \`inscripcion\` add index \`inscripcion_torneo_id_index\`(\`torneo_id\`);`);
    this.addSql(`alter table \`inscripcion\` add index \`inscripcion_equipo_id_index\`(\`equipo_id\`);`);

    this.addSql(`create table \`usuario\` (\`id\` int unsigned not null auto_increment primary key, \`email\` varchar(255) not null, \`password\` varchar(255) not null, \`nombre\` varchar(255) not null, \`apellido\` varchar(255) not null, \`role\` enum('administrador', 'gestor', 'recepcionista', 'arbitro', 'jugador') not null, \`activo\` tinyint(1) not null default true, \`ultimo_login\` datetime null, \`documento\` varchar(255) null, \`telefono\` varchar(255) null, \`fecha_nacimiento\` datetime null, \`nivel_acceso\` varchar(255) null, \`departamento\` varchar(255) null, \`turno\` varchar(255) null, \`categoria\` varchar(255) null, \`numero_licencia\` varchar(255) null, \`especialidad\` varchar(255) null, \`posicion\` varchar(255) null, \`numero_camiseta\` int null, \`equipo_id\` int null) default character set utf8mb4 engine = InnoDB;`);
    ...
    this.addSql(`create table \`partido\` (\`id\` int unsigned not null auto_increment primary key, \`fecha\` date null, \`torneo_id\` int unsigned not null, \`equipo1_id\` int unsigned not null, \`equipo2_id\` int unsigned not null, \`sede_id\` int unsigned null, \`arbitro_id\` int unsigned null, \`goles_equipo1\` int null, \`goles_equipo2\` int null, \`estado\` varchar(255) not null default 'pendiente') default character set utf8mb4 engine = InnoDB;`);
